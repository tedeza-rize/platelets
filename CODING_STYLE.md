...
17.1 컴포넌트는 기능 폴더로 묶는다

"src/components" 아래 파일을 평평하게 두지 않는다. 컴포넌트는 자신의 기능
폴더(map, dashboard, setup, admin, ai, auth, field, settings, system,
feedback, licenses)에 둔다. 각 컴포넌트는 같은 폴더에 자신의 스타일 모듈과
로컬 헬퍼를 함께 둔다(콜로케이션).

다른 기능 폴더의 컴포넌트는 "@/components/<기능>/<이름>" 별칭으로 가져온다.
여러 폴더가 공유하는 콘솔 스타일(admin/management-console.module.scss,
admin/role-console.module.scss)도 상대 경로 대신 이 별칭으로 가져온다.

새 컴포넌트는 알맞은 기능 폴더에 추가하고, 정말 새로운 기능 영역이 생길 때만
새 폴더를 만든다. "src/components" 루트에 낱개 파일을 두지 않는다.

17.2 스타일은 CSS가 아니라 SCSS를 사용한다

모든 컴포넌트·페이지 스타일은 SCSS 모듈("*.module.scss")로 작성하고
컴포넌트와 같은 폴더에 둔다. 전역 스타일은 "src/app/globals.scss"이며 KRDS
디자인 토큰을 정의한다(DESIGN.md 참고). 새 ".css" 파일을 만들지 않는다.
색·간격·라운드·그림자 등은 globals.scss의 "--krds-*" 토큰을 사용하고
하드코딩 hex를 새로 추가하지 않는다.
...
- 컴포넌트는 기능 폴더로 묶고, 스타일은 SCSS 모듈("*.module.scss")로 작성한다.
